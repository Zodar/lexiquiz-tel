...
//				if(j  + 1 < l.get(i).length) System.out.println( l.get(i)[j + 1] );
	 ...
	 * Si reçoit -1 pour le niveau, renvoi toutes les questions.
	...
	public int countQuestion(int idQuiz, int niveau) {
		int count = 0;
		if(niveau != -1){
			if(getAllQuestion(idQuiz, niveau) != null) count += getAllQuestion(idQuiz, niveau).size();
		}
		else{
			for (int i = 0; i < 4; i++){
				if(getAllQuestion(idQuiz, i) != null) count += getAllQuestion(idQuiz, i).size();
			}
		}
		return count;
	}
